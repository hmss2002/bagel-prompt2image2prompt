...
# -*- coding: utf-8 -*-
"""
Caption images using InstructBLIP (Image-to-Text).
...
def to_tensor_dict(inputs: Dict[str, Any], device: str) -> Dict[str, torch.Tensor]:
    tensor_inputs: Dict[str, torch.Tensor] = {}
    for k, v in inputs.items():
        if isinstance(v, list):
            if v and isinstance(v[0], np.ndarray):
                arr = np.array(v)
                tensor_inputs[k] = torch.tensor(arr)
            else:
                tensor_inputs[k] = torch.tensor(v)
        else:
            tensor_inputs[k] = v
        if isinstance(tensor_inputs[k], torch.Tensor):
            tensor_inputs[k] = tensor_inputs[k].to(device)
...
def main() -> None:
    ...
    caption_prompt = args.caption_prompt or cfg.get("caption_prompt", "")
    max_new_tokens = args.max_new_tokens if args.max_new_tokens is not None else cfg.get("max_new_tokens", 160)
    ...
    if not model_path:
        raise RuntimeError("i2t_model_path is required for InstructBLIP captioning.")
    ...
    try:
        ...
        raise RuntimeError("transformers with InstructBLIP is required.") from e
    ...
    for row in my_rows:
        ...
        try:
            ...
            raw_inputs = processor(images=img, text=caption_prompt or "Describe this image.")
            inputs = to_tensor_dict(raw_inputs, device)
            out_ids = model.generate(**inputs, max_new_tokens=max_new_tokens)
            inferred = processor.batch_decode(out_ids, skip_special_tokens=True)[0].strip()
            if not inferred:
                raise RuntimeError("No text returned by InstructBLIP.")
